...
// subscriber receives the same issue; only the unsubscribe link differs.
import { createServiceClient } from '@/lib/supabase/server'
import { DIRECTORY } from '@/lib/data/directory'
import { composeIssue, summarizeMarket, SIGNAL_BASELINE_WEEKS, DigestSourceArticle, Issue } from '@/lib/email/digest'
...
export async function buildIssue(
  ...
  const [prior, facts, quotes, directory] = await Promise.all([
    service
      .from('articles')
      .select('signal_ids')
