...
lib/
  ...
  feedGenerator.ts       — Feed generator: scrapes an HTML listing page (cheerio + CSS selectors) or
                           ...
                           RSS XML served at /api/feeds/[name]. Intended to replace the external
                           aparasion/rss-generator, but the cutover has NOT happened — see scraped_sources
...
**This generator was built to replace the external `aparasion/rss-generator`, and that cutover has not been made.** As of 2026-09-14 there are 13 active `scraped_sources` regenerating daily, and **zero** `rss_sources` rows pointing at `/api/feeds/<name>` — so nothing consumes their output. Ingest instead reads 11 active feeds from the external generator, which now lives on **GitLab Pages** (`https://aparasion.gitlab.io/rss-generator/rss/<Name>.xml`), regenerated at `0 6,9,12,14,16 * * 1-5` Europe/Warsaw — GitLab CI schedules are timezone-aware, so it needs no DST-pair trick. One older `aparasion.github.io` row survives, inactive.

Ten of the 13 scrape sources duplicate a GitLab feed by name (`crowdin-blog`, `GALA-Global`, `LingopalAI`, `lokalise-blog`, `OpenAI-News-L10N`, `phrase-blog`, `PRNewswire-L10N`, `TAUS-Blog`, `transperfect-news-and-press`, `XTM-Blog`). Three exist only here and therefore reach no draft: `Cohere-Newsroom-L10N`, `RWS-media-centre` (RWS is covered only indirectly, via a Google News search feed), and `DATAmundi-Newsroom` (whose scrape also reports `last_status='error'`). Conversely `DeepL-Press-Releases` exists only on GitLab, with no scrape source configured.

Before changing either generator, check which one actually feeds `rss_sources` — editing a `scraped_sources` row has no effect on ingest today.
...
| `0 6,9,12,14,16 * * 1-5` Europe/Warsaw | **GitLab CI** (external `aparasion/rss-generator`, outside this repo) | Regenerates the feeds at `https://aparasion.gitlab.io/rss-generator/rss/<Name>.xml` that 11 active `rss_sources` rows point at. Not part of the GitHub→Vercel setup — from ingest's side these are just ordinary feed URLs. The 09:00/12:00/16:00 runs each land ~1h before an ingest run |
