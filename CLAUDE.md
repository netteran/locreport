...
Two rows are deliberate exceptions:

- **`DeepL Press Releases`** still points at the old GitLab Pages generator (`https://aparasion.gitlab.io/rss-generator/rss/DeepL-Press-Releases.xml`, regenerated `0 6,9,12,14,16 * * 1-5` Europe/Warsaw — GitLab CI schedules are timezone-aware, so it needs no DST-pair trick). It has no `scraped_sources` equivalent, so it was left alone rather than dropped. **The GitLab project cannot be retired until a `DeepL-Press-Releases` scrape source exists.**
- **`DATAmundi Newsroom`** is present but `active=false`. Its scrape fails with `HTTP 403` — datamundi.ai blocks the scraper's request, so `generated_xml` is empty and `/api/feeds/DATAmundi-Newsroom` would 404 on every run. This is a bot-protection problem, not a selector problem; activate the row once the fetch succeeds.

One older `aparasion.github.io` row also survives, inactive.
