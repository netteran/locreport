...
Its `url` (`https://www.deepl.com/en/press`) is the one unverified field — the listing path could not be checked from the environment the source was added in. A wrong path surfaces immediately as `last_error: HTTP 404`.
...
A `/api/feeds/<name>` row only works if the scrape source is `active` **and** has non-empty `generated_xml` — otherwise the route 404s. When adding one, run the generator first and confirm `last_status='success'`.
