...
One row is still a deliberate exception: **`DeepL Press Releases`** reads the old GitLab Pages feed (`https://aparasion.gitlab.io/rss-generator/rss/DeepL-Press-Releases.xml`, regenerated `0 6,9,12,14,16 * * 1-5` Europe/Warsaw — GitLab CI schedules are timezone-aware, so it needs no DST-pair trick). A `DeepL-Press-Releases` scrape source now exists and is active, but the `rss_sources` row stays on GitLab until that scrape has produced items — repointing a working feed at an unproven scrape would silently lose coverage. **Once `/admin/scraped-sources` shows it `success` with a non-zero item count, repoint the row to `/api/feeds/DeepL-Press-Releases` and the GitLab project can be retired.**

**Its scrape does not work yet.** A run against `https://www.deepl.com/en/press` returned `success` with **0 items** — so the fetch got HTTP 200, but the response contained no `<article>` elements *and* the anchor fallback found not one `/press-release/` href anywhere on the page. That rules out a selector mistake (the selectors are verified below) and leaves two candidates: the listing lives at a different path, or the page renders client-side so the server HTML is an empty shell. The URL is now set to `https://www.deepl.com/en/press-release`, the path the detail pages sit under, as the next candidate.

To tell those apart: open the page's **View Source** (Ctrl+U — the raw response), not the DevTools Elements panel (the post-JS DOM, which always looks populated), and search for `press-release`. No match in View Source means client-rendered, and no selector config can ever scrape it — the feed would need the Next.js RSC payload parsed, or a headless browser. The external GitLab generator does produce this feed with the same fetch+cheerio approach, so **its `config.json` entry is the authoritative answer** to both the URL and the technique.

**A scrape that yields 0 items fails silently.** It still stores a valid, empty RSS channel (~485 bytes), so `/api/feeds/<name>` returns 200 and ingest reads a feed with no entries — no error anywhere. Always check `last_item_count`, not just `last_status`, before trusting a source.

Its selectors (`article` / `h3` / `a[href*='/press-release/']` / `time`, pattern `/press-release/`) were validated against the live markup and use **no class names at all** — DeepL's page is built from Tailwind utilities and hashed CSS-module names (`richText-module__sMfSca__…`) that rotate on every frontend deploy. Prefer semantic elements over classes on any DeepL-like source. The `<time>` element carries no `datetime` attribute, so the date comes from its text ("September 1, 2026"), which `parseArticleDate` handles natively.
...
|---|---|---|---|
| `LingopalAI` | 0 | no | Broken. Its `rss_sources` row was reverted to the GitLab feed, since an empty feed is strictly worse than the one it replaced |
